...
for report_id in $report_ids; do
    # Extract the query generator for the current report
    queryGenerator=$(echo "$report_data" | jq -r ".[\"$report_id\"].query.queryGenerator")

    if [ -z "$queryGenerator" ]; then
        echo "Error: Query generator is not available for report ID $report_id"
        continue
    fi

    # URL-encode the query generator
    encoded_queryGenerator=$(echo "$queryGenerator" | jq -sRr @uri)

    # Construct the complete URI
    uri=$(printf "$uri_template" "$encoded_queryGenerator")

    # Generate the timestamp and outfile
    timestamp=$(date "+%Y-%m-%d %H%M%S")
    outfile="$reportFolder/report_$timestamp.$format"

    echo "$timestamp - Generating report from URI: $uri"
    echo "$timestamp - Output file: $outfile"
