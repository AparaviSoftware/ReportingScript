...
for report_id in $report_ids; do
    ...
    uri_encoded_query=$(echo "$report_data" | jq -r ".[\"$report_id\"].query.queryGenerator" | jq -sRr @uri)

    uri=$(printf "$uri_template" "$uri_encoded_query")

    # Remove the unwanted substring from the URI
    uri=$(echo "$uri" | sed 's/query?select=\(.*\)SELECT/query?select=SELECT/g')
